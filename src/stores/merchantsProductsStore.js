import {
  ...
  export const useMerchantsProductsStore = defineStore("merchants-products", {
    ...
    actions: {
      ...
      async createMerchantProduct(productData, imageFiles) {
        try {
          ...
          const imageUrls = await Promise.all(
            filesArray.map(async (file) => {
              const storagePath = `/merchants-products/${file.name}`;
          ...
          const docRef = await addDoc(collection(db, "merchants-products"), newProductData);
      ...
      async updateMerchantProduct(productId, updatedData, imageFiles) {
        try {
          const filesArray = Object.values(imageFiles).filter((file) => file);
          const imageUrls = await Promise.all(
            filesArray.map(async (file) => {
          ...
          const productRef = doc(db, "merchants-products", productId);
      ...
      async deleteMerchantProduct(productId) {
        if (!productId) return;
        try {
          const productRef = doc(db, "merchants-products", productId);
      ...
      async fetchMerchantLowStockProducts() {
        try {
          const productsRef = collection(db, "merchants-products");
