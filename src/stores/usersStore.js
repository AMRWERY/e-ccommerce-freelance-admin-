...
export const useUsersStore = defineStore('users', {
  ...
  actions: {
    ...
    calculateUserStats(user, orders) {
      const userOrders = orders.filter(order => order.deliveryDetails?.email === user.email)
    ...
    async fetchEmployees() {
      ...
      try {
        ...
        this.employees = snapshot.docs.map(doc => this.formatEmployeeData(doc))
      } catch (error) {
    ...
    async fetchDashboardUsers() {
      ...
      try {
        ...
        this.activeUsers = users.map(user => this.calculateUserStats(user, this.orders))
      } catch (error) {
